...
public class ApiLoginExtension implements BeforeEachCallback, AfterTestExecutionCallback, ParameterResolver {
  ...
  public void beforeEach(ExtensionContext extensionContext) throws Exception {
    ...
    if (apiLogin.isPresent()) {
        ...
        String login = createdUserForApiLogin.username();
        String password = createdUserForApiLogin.testData().password();
  ...
  private static UserJson getCreatedUserForApiLogin(ExtensionContext extensionContext) {
    return (UserJson) extensionContext.getStore(CreateUserExtension.DB_CREATE_USER_NAMESPACE).get(extensionContext.getUniqueId(), Map.class)
        .get(0);
  }
